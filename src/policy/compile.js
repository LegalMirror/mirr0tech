...
import { validateCashier, bindCashierClauses, emitCashierTerms } from './cashier.js';
...
function checkCustodialConfig(config, ast, { secondary = false } = {}) {
  ...
  if (config.decimals !== 6 || config.custody !== 'backend' || config.currency !== 'USD' || !(config.priceModel === 'one-token-per-usd' || (secondary && config.cashier?.enabled && config.priceModel === 'fixed-nav'))) throw new Error('Only the six-decimal custodial USD or opt-in fixed-NAV demo price model is implemented');
...
export function compilePolicy(envelope, config, document, { demo = false } = {}) {
  ...
  const cashier = validateCashier(envelope.ast, config, document, demo);
  const resolution = resolveComponents(envelope.ast, config);
  const onchain = buildOnchainPolicy(envelope.ast);
  bindCashierClauses(onchain, envelope.ast, cashier);
  ...
  const solidity = profile !== 'wildcat-credit' ? `// SPDX-License-Identifier: UNLICENSED
...
` : null;
  ...
  return { policy, cashier, solidity, javascript, compiledPolicy, compiledCashierTerms: emitCashierTerms(cashier), clauseTable, onchain, components: resolution.manifest, contracts: resolution.contracts, equivalenceChecks: checked };
}
