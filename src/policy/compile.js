...
const PROFILES = ['custodial-rwa', 'wildcat-credit'];
...
function checkCustodialConfig(config, ast) {
  if (ast.rules.some((r) => r.action === 'transfer' && r.effect === 'permit')) throw new Error('Transfer permissions are unsupported by the custodial MVP');
...
export function compilePolicy(envelope, config, document, { demo = false } = {}) {
  ...
  const solidity = profile === 'custodial-rwa' ? `// SPDX-License-Identifier: MIT
