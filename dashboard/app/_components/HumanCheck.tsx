...
/**
 * "Verify with World ID" for a wallet. With a registered app the World ID widget opens and the
 * proof goes to the gateway; without one, a mock proof is submitted so the flow still runs.
...
export function HumanCheck({
  ...
  if (party.facts.humanVerified === true && !asParty) return null;
  ...
  return (
    <span className="human-check">
      <button
        ...
        disabled={busy}
        onClick={start}
        title="World ID proof of human, bound to this wallet"
      >
      ...
      {context && !context.mock && (
        <IDKitRequestWidget
          ...
          allow_legacy_proofs
          preset={proofOfHuman({ signal: party.address })}
          open
