...
```sh
npm run dev:stack   # anvil + deployed stack + operator API at http://127.0.0.1:$PORT/v1/stack (PORT default 3000; the Compose stack uses 3200)
```
...
| POST | `/rwa/mint` `{amount}`, `/rwa/release` `{wallet, amount}`, `/rwa/pools` `{wallet, hooked}`, `/rwa/liquidity`, `/rwa/swap` | the fund token's venue over the base stack; the flow runs the same routes per agreement under `/v1/agreements/:id/stack/*` |
| POST/GET | `/credit/deposit`, `/credit/withdraw` `{wallet, amount}`; `/credit/buyback` (ship / read: program disassembled, hash chain, Aqua balances); `/credit/buyback/quote`, `/fill` `{wallet, amount}`, `/dock` | the credit venue (out of the demo flow) |
...
`POST /webhooks/payments` takes a payment rail's event (Stripe shape; a wire notification in the same shape works) signed with `PAYMENT_WEBHOOK_SECRET` under `Stripe-Signature` (`t=<unix>,v1=<HMAC-SHA256 of "<t>.<raw body>">`, 5-minute tolerance). No bearer: the signature is the credential. A settled USD event (`payment_intent.succeeded`, `charge.succeeded`, `wire.received`) with `metadata.wallet` (and `metadata.agreement` to settle on that agreement's token; the stack's fund token otherwise) attests `depositConfirmed` for that wallet (merged into its facts), then asks the policy about `mint`: allowed → mint to custody and release to the wallet; refused → the money is **held** and the audit names the sentence; a chain error (supply cap, gas) is `failed` and the rail's retry tries again. Same event id settles once (`replay: true`). Other event types answer `{ received: true, ignored }`. Always 2xx once the signature verifies, so the rail never retries a policy decision. `src/payments.js`, `VenueService.settlePayment`; tests `test/payments.test.js`, `test/chain/payments.test.js`.

## Operator API (custodial issuance)

The original MVP's REST gateway (`npm start`, bearer token, `Idempotency-Key`, restart recovery) still drives the fund token's custodial issuance: `GET /v1/policy`, `POST /v1/investors`, `POST /v1/deposits`, `POST /v1/mints`, `POST /v1/redemptions`, `GET /v1/audit`, plus mock compliance/settlement endpoints. `npm run demo` exercises it on a mock chain; `CHAIN_MODE=evm` uses the deployed token (`.env.example`).

## Repository map

| Path | Role |
| --- | --- |
| `src/policy/` | `document.js` normalization + bundles · `schema.js` facts, actions, AST schema · `dnf.js` DNF + equivalence · `onchain.js` emits `CompiledPolicy.sol` · `compile.js` profiles, proof, hashes · `programs.js` SwapVM programs/orders · `fixture.js`, `mla-fixture.js` hand-authored ASTs (the drafts a deliberation starts from) · `extract.js` legacy single-model extractor (`npm run extract`, not in the flow) · `hookAddress.js` CREATE2 mining |
| `src/noolog/` | generation by Noolog deliberation: `client.js` gateway calls · `extract.js` document → AST proposal · `verify.js` per-claim verdicts and confidence · `mock.js` the in-process mock without `NOOLOG_API_KEY` |
| `src/agreements.js`, `src/agreements-api.js`, `scripts/mirr0.js` | the flow: agreement store and lifecycle (upload → generate → verify → compile → constrain → deploy), its `/v1/agreements*` routes, the CLI ([AGREEMENTS_API.md](AGREEMENTS_API.md)) |
...
| `src/deploy.js`, `scripts/deploy-stack.js` | one call deploys the base stack (fund venue + credit venue) against any RPC; `deployFund` deploys an uploaded agreement's oracle, token, hook and pool (anvil default; canonical `POOL_MANAGER`/`AQUA`/`WETH` via env on a public chain) |
...
| `scripts/demo-golden.js`, `scripts/demo-credit.js`, `scripts/demo.js` | the fund flow and the credit venue on a fresh anvil · the credit venue's lender lifecycle · the original custodial issuance demo (REST) |
| `test/`, `test/chain/` | unit tests incl. the equivalence proof · anvil tests: `flow` (the CLI flow end to end), `agreements-deploy`, `payments`, `signing`, `evm` (token + operator signer), `rwa-pool` (fund token + hook), `hook` (v4), `stack`, `gateway`, `credit` (role provider), `swapvm` (Aqua strategy) |
| `docs/` | `PRD.md`, `AGREEMENTS_API.md`, `ARCHITECTURE.md`, `WORLD_ID.md`, `NOOLOG.md`, `DEMO_SCRIPT.md`; credit venue: `SWAPVM_INTEGRATION.md`, `MLA_CLAUSE_MAP.md` |
