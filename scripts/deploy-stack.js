...
import { deployStack, ANVIL_CHAIN_ID, ANVIL_DEV_KEY } from '../src/deploy.js';

const provider = new JsonRpcProvider(process.env.RPC_URL ?? 'http://127.0.0.1:8545', undefined, { cacheTimeout: -1 });
try {
  const chainId = (await provider.getNetwork()).chainId;
  const key = process.env.DEPLOYER_PRIVATE_KEY ?? (chainId === ANVIL_CHAIN_ID ? ANVIL_DEV_KEY : undefined);
  if (!key) throw new Error(`Set DEPLOYER_PRIVATE_KEY for chain ${chainId}; only anvil (31337) has a default key`);
