...
| | |
| --- | --- |
| **Status** | **v2** — supersedes v1 (`PRD_v1.pdf`). Decisions are locked unless marked ⚠️. **Sat 05:00 status:** G1–G6 green on anvil and deployed + seeded on Sepolia (canonical Aqua, PoolManager; fixed bid and tender offer posted); dashboard PM-first and public on GitHub Pages; submission text drafted. Open: MultiBaas registration (needs the API key), public live gateway (Coolify creds), third prize, team names, video |
| **Owner** | Lam (PM) |
...
| **Sponsor tracks** | One prize per company. **1inch — Build an Aqua App** (P0) · **Curvegrid — Best RWA Tokenization Project** (P0; ⚠️ or Digital Asset Dashboard, Lam picks) · **World** (P0 since Sat: proof of human as the KYC fact that gates issuance and the policy-managed pool) · Uniswap not submitted · Intercepta dropped |
...
| **Sat 24:00** | Done early: dashboard complete and public (GitHub Pages, auto-deploy on push); Sepolia deployed and seeded, timeline and lender snapshot shipped with the static site; Coolify compose ready for a public live gateway when creds exist | Lam runs the golden path from the UI 2× |
