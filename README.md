...
- **Two positions from one addendum.** `BuybackFixedPrice` (the standing bid at the A1.1 price) and `BuybackDutchAuction` — the official `DutchAuctionBalanceOut` between the rate and the curve, so the borrower's bid opens at the floor and improves to the A1.5 ceiling over the window, a tender offer with the agreement as an opcode (`POST /v1/stack/credit/buyback {auction: true}`).
- **On-chain execution.** `test/chain/swapvm.test.js` and `scripts/demo-golden.js`: `ship` → `quote` → `swap` with `pull`/`push` → refusals → cap and deadline → `dock`; the auction test quotes at open, midway and near the close, fills, then expires.
