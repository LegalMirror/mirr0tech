...
import { confidenceLabel, verificationSentence } from "@/lib/verification";
    ...
    <section className="card act-card">
      ...
      {policy.verification && (
        <p className="meta" title={verificationSentence(policy.verification)}>
          Confidence {policy.verification.confidence.overall.toFixed(2)} (
          {confidenceLabel(policy.verification.confidence.overall)}) ·{" "}
          {policy.verification.confidence.counts.contested} contested
        </p>
      )}
