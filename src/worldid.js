// World ID: a proof of human becomes the `humanVerified` fact. The verifier talks to World's
...
export function mockProof(wallet, { action = DEFAULT_ACTION, nullifier = null } = {}) {
  return {
    ...
    responses: [{ identifier: 'mock', issuer_schema_id: 1, nullifier: nullifier ?? sha(`human:${wallet.toLowerCase()}`), expires_at_min: 0,
      proof: ['0x1', '0x2', '0x3', '0x4', '0x5'], signal_hash: sha(`signal:${wallet.toLowerCase()}`) }],
  };
}

export class WorldIdVerifier {
  constructor({ rpId = process.env.WORLD_RP_ID ?? process.env.WORLD_APP_ID ?? null, action = process.env.WORLD_ACTION ?? DEFAULT_ACTION,
    environment = process.env.WORLD_ENV ?? 'staging', signingKeyHex = process.env.WORLD_RP_SIGNING_KEY ?? null, url = WORLD_VERIFY_URL, fetchImpl = fetch } = {}) {
    Object.assign(this, { rpId, action, environment, signingKeyHex, url, fetchImpl });
  }
  ...
  async context() {
    ...
    return { app_id: this.rpId ?? 'app_mock', rp_id: rp_context.rp_id, action: this.action, environment: this.mock ? 'mock' : this.environment, mock: this.mock, rp_context };
  }

  /// Verifies a proof payload; returns { success, nullifier, action, environment }.
