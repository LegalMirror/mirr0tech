...
describe("fixture provenance", () => {
  it("does not fabricate model verification for offline compiler exports", async () => {
    const policy = await compiled("wildcat-credit");
    expect(policy.verification).toBeNull();
    expect(policy.extraction.provider).toBe("demo");
  });
});
