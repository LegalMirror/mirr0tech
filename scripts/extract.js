...
const result = fixture ? sampleFixture(document) : await extractAst(document, { apiKey: process.env.OPENAI_API_KEY, model: process.env.OPENAI_MODEL });
await mkdir(dirname(destination), { recursive: true });
await writeFile(destination, `${JSON.stringify(result, null, 2)}\n`);
console.log(`Wrote ${destination} (${result.extraction.provider}; ${result.ast.rules.length} rules; ${result.ast.unresolved.length} unresolved terms)`);
